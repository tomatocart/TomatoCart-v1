var popDialog=new Class({Implements:[Options],options:{clsEl:"dialog",enableAnimation:true,baseClsAnimated:"animated",clsAnimated:"bounceInDown",clsClose:"btnClose",clsHideAnimation:"slideOutUp"},initialize:function(c,d){this.setOptions(d);this.btnClose=this.createCloseBtn();this.dlg=this.createDialog(c);this.currentClsAnimated=this.options.clsAnimated;this.showed=false},show:function(){if(this.showed){if(this.options.enableAnimation===true){this.dlg.removeClass(this.options.clsAnimated);this.dlg.removeClass(this.currentClsAnimated)}this.dlg.setStyle("display","block");this.setPosition();if(this.options.enableAnimation===true){this.dlg.addClass(this.options.clsAnimated)}}else{$(document.body).adopt(this.dlg);this.setPosition();if(this.options.enableAnimation===true){this.dlg.addClass(this.options.baseClsAnimated);this.dlg.addClass(this.options.clsAnimated)}this.showed=true}},setPosition:function(){var e=window,j="inner",a,i,h;if(!("innerWidth" in window)){j="client";e=document.documentElement||document.body}a={width:e[j+"Width"],height:e[j+"Height"]};h=window.pageYOffset||document.body.scrollTop||document.documentElement.scrollTop;i=this.dlg.getSize();this.dlg.setStyles({left:((a.width/2-i.x/2)+"px"),top:((a.height/2-i.y/2+h)+"px")})},createDialog:function(d){var c;if(typeof d!="undefined"){c=new Element("div",{"class":this.options.clsEl,html:d,styles:{position:"absolute"}});c.adopt(this.btnClose);return c}return null},createCloseBtn:function(){var b=new Element("a",{href:"#",html:"&#10006;","class":this.options.clsClose});b.addEvent("click",function(a){a.stop();this.doAnimate(this.options.clsHideAnimation);(function(){this.hide()}.bind(this)).delay(500);return false}.bind(this));return b},doAnimate:function(b){if(b&&(b!=this.options.clsAnimated)){this.dlg.removeClass(this.options.clsAnimated);this.dlg.addClass(b);this.currentClsAnimated=b}},hide:function(){this.dlg.setStyle("display","none")},update:function(b){this.dlg.set("html",b);this.dlg.adopt(this.btnClose)},destroy:function(){this.dlg.destroy()}});var Autocompleter=new Class({Implements:[Options,Events],options:{minLength:1,markQuery:true,width:330,maxChoices:10,injectChoice:null,customChoices:null,emptyChoices:null,visibleChoices:true,className:"autocompleter-choices",zIndex:42,delay:400,observerOptions:{},fxOptions:{},autoSubmit:false,overflow:false,overflowMargin:25,selectFirst:false,filter:null,filterCase:false,filterSubset:false,forceSelect:false,selectMode:true,choicesMatch:null,multiple:false,separator:", ",separatorSplit:/\s*[,;]\s*/,autoTrim:false,allowDupes:false,cache:true,relative:false},initialize:function(d,e){this.element=$(d);this.setOptions(e);this.build();this.observer=new Observer(this.element,this.prefetch.bind(this),$merge({delay:this.options.delay},this.options.observerOptions));this.queryValue=null;if(this.options.filter){this.filter=this.options.filter.bind(this)}var f=this.options.selectMode;this.typeAhead=(f=="type-ahead");this.selectMode=(f===true)?"selection":f;this.cached=[]},build:function(){if($(this.options.customChoices)){this.choices=this.options.customChoices}else{this.choices=new Element("ul",{"class":this.options.className,styles:{zIndex:this.options.zIndex}}).inject(document.body);this.relative=false;if(this.options.relative){this.choices.inject(this.element,"after");this.relative=this.element.getOffsetParent()}this.fix=new OverlayFix(this.choices)}if(!this.options.separator.test(this.options.separatorSplit)){this.options.separatorSplit=this.options.separator}this.fx=(!this.options.fxOptions)?null:new Fx.Tween(this.choices,$merge({property:"opacity",link:"cancel",duration:200},this.options.fxOptions)).addEvent("onStart",Chain.prototype.clearChain).set(0);this.element.setProperty("autocomplete","off").addEvent((Browser.Engine.trident||Browser.Engine.webkit)?"keydown":"keypress",this.onCommand.bind(this)).addEvent("click",this.onCommand.bind(this,[false])).addEvent("focus",this.toggleFocus.create({bind:this,arguments:true,delay:100})).addEvent("blur",this.toggleFocus.create({bind:this,arguments:false,delay:100}))},destroy:function(){if(this.fix){this.fix.destroy()}this.choices=this.selected=this.choices.destroy()},toggleFocus:function(b){this.focussed=b;if(!b){this.hideChoices(true)}this.fireEvent((b)?"onFocus":"onBlur",[this.element])},onCommand:function(c){if(!c&&this.focussed){return this.prefetch()}if(c&&c.key&&!c.shift){switch(c.key){case"enter":if(this.element.value!=this.opted){return true}if(this.selected&&this.visible){this.choiceSelect(this.selected);return !!(this.options.autoSubmit)}break;case"up":case"down":if(!this.prefetch()&&this.queryValue!==null){var d=(c.key=="up");this.choiceOver((this.selected||this.choices)[(this.selected)?((d)?"getPrevious":"getNext"):((d)?"getLast":"getFirst")](this.options.choicesMatch),true)}return false;case"esc":case"tab":this.hideChoices(true);break}}return true},setSelection:function(m){var l=this.selected.inputValue,k=l;var r=this.queryValue.length,p=l.length;if(l.substr(0,r).toLowerCase()!=this.queryValue.toLowerCase()){r=0}if(this.options.multiple){var n=this.options.separatorSplit;k=this.element.value;r+=this.queryIndex;p+=this.queryIndex;var q=k.substr(this.queryIndex).split(n,1)[0];k=k.substr(0,this.queryIndex)+l+k.substr(this.queryIndex+q.length);if(m){var o=k.split(this.options.separatorSplit).filter(function(a){return this.test(a)},/[^\s,]+/);if(!this.options.allowDupes){o=[].combine(o)}var j=this.options.separator;k=o.join(j)+j;p=k.length}}this.observer.setValue(k);this.opted=k;if(m||this.selectMode=="pick"){r=p}this.element.selectRange(r,p);this.fireEvent("onSelection",[this.element,this.selected,k,l])},showChoices:function(){var t=this.options.choicesMatch,u=this.choices.getFirst(t);this.selected=this.selectedValue=null;if(this.fix){var r=this.element.getCoordinates(this.relative),v=this.options.width||"auto";this.choices.setStyles({left:r.left,top:r.bottom,width:(v===true||v=="inherit")?r.width:v})}if(!u){return}if(!this.visible){this.visible=true;this.choices.setStyle("display","");if(this.fx){this.fx.start(1)}this.fireEvent("onShow",[this.element,this.choices])}if(this.options.selectFirst||this.typeAhead||u.inputValue==this.queryValue){this.choiceOver(u,this.typeAhead)}var s=this.choices.getChildren(t),q=this.options.maxChoices;var n={overflowY:"hidden",height:""};this.overflown=false;if(s.length>q){var m=s[q-1];n.overflowY="scroll";n.height=m.getCoordinates(this.choices).bottom;this.overflown=true}this.choices.setStyles(n);this.fix.show();if(this.options.visibleChoices){var o=document.getScroll(),l=document.getSize(),p=this.choices.getCoordinates();if(p.right>o.x+l.x){o.x=p.right-l.x}if(p.bottom>o.y+l.y){o.y=p.bottom-l.y}window.scrollTo(Math.min(o.x,p.left),Math.min(o.y,p.top))}},hideChoices:function(e){if(e){var f=this.element.value;if(this.options.forceSelect){f=this.opted}if(this.options.autoTrim){f=f.split(this.options.separatorSplit).filter($arguments(0)).join(this.options.separator)}this.observer.setValue(f)}if(!this.visible){return}this.visible=false;if(this.selected){this.selected.removeClass("autocompleter-selected")}this.observer.clear();var d=function(){this.choices.setStyle("display","none");this.fix.hide()}.bind(this);if(this.fx){this.fx.start(0).chain(d)}else{d()}this.fireEvent("onHide",[this.element,this.choices])},prefetch:function(){var k=this.element.value,l=k;if(this.options.multiple){var n=this.options.separatorSplit;var i=k.split(n);var h=this.element.getSelectedRange().start;var j=k.substr(0,h).split(n);var m=j.length-1;h-=j[m].length;l=i[m]}if(l.length<this.options.minLength){this.hideChoices()}else{if(l===this.queryValue||(this.visible&&l==this.selectedValue)){if(this.visible){return false}this.showChoices()}else{this.queryValue=l;this.queryIndex=h;if(!this.fetchCached()){this.query()}}}return true},fetchCached:function(){return false;if(!this.options.cache||!this.cached||!this.cached.length||this.cached.length>=this.options.maxChoices||this.queryValue){return false}this.update(this.filter(this.cached));return true},update:function(c){this.choices.empty();this.cached=c;var d=c&&$type(c);if(!d||(d=="array"&&!c.length)||(d=="hash"&&!c.getLength())){(this.options.emptyChoices||this.hideChoices).call(this)}else{if(this.options.maxChoices<c.length&&!this.options.overflow){c.length=this.options.maxChoices}c.each(this.options.injectChoice||function(a){var b=new Element("li",{html:this.markQueryValue(a)});b.inputValue=a;this.addChoiceEvents(b).inject(this.choices)},this);this.showChoices()}},choiceOver:function(n,m){if(!n||n==this.selected){return}if(this.selected){this.selected.removeClass("autocompleter-selected")}this.selected=n.addClass("autocompleter-selected");this.fireEvent("onSelect",[this.element,this.selected,m]);if(!this.selectMode){this.opted=this.element.value}if(!m){return}this.selectedValue=this.selected.inputValue;if(this.overflown){var k=this.selected.getCoordinates(this.choices),l=this.options.overflowMargin,j=this.choices.scrollTop,i=this.choices.offsetHeight,h=j+i;if(k.top-l<j&&j){this.choices.scrollTop=Math.max(k.top-l,0)}else{if(k.bottom+l>h){this.choices.scrollTop=Math.min(k.bottom-i+l,h)}}}if(this.selectMode){this.setSelection()}},choiceSelect:function(b){if(b){this.choiceOver(b)}this.setSelection(true);this.queryValue=false;this.hideChoices()},filter:function(b){return(b||this.tokens).filter(function(a){return this.test(a)},new RegExp(((this.options.filterSubset)?"":"^")+this.queryValue.escapeRegExp(),(this.options.filterCase)?"":"i"))},markQueryValue:function(b){return(!this.options.markQuery||!this.queryValue)?b:b.replace(new RegExp("("+((this.options.filterSubset)?"":"^")+this.queryValue.escapeRegExp()+")",(this.options.filterCase)?"":"i"),'<span class="autocompleter-queried">$1</span>')},addChoiceEvents:function(b){return b.addEvents({mouseover:this.choiceOver.bind(this,[b]),click:this.choiceSelect.bind(this,[b])})}});var OverlayFix=new Class({initialize:function(b){if(Browser.Engine.trident){this.element=$(b);this.relative=this.element.getOffsetParent();this.fix=new Element("iframe",{frameborder:"0",scrolling:"no",src:"javascript:false;",styles:{position:"absolute",border:"none",display:"none",filter:"progid:DXImageTransform.Microsoft.Alpha(opacity=0)"}}).inject(this.element,"after")}},show:function(){if(this.fix){var b=this.element.getCoordinates(this.relative);delete b.right;delete b.bottom;this.fix.setStyles($extend(b,{display:"",zIndex:(this.element.getStyle("zIndex")||1)-1}))}return this},hide:function(){if(this.fix){this.fix.setStyle("display","none")}return this},destroy:function(){if(this.fix){this.fix=this.fix.destroy()}}});Element.implement({getSelectedRange:function(){if(!Browser.Engine.trident){return{start:this.selectionStart,end:this.selectionEnd}}var h={start:0,end:0};var g=this.getDocument().selection.createRange();if(!g||g.parentElement()!=this){return h}var j=g.duplicate();if(this.type=="text"){h.start=0-j.moveStart("character",-100000);h.end=h.start+g.text.length}else{var f=this.value;var i=f.length-f.match(/[\n\r]*$/)[0].length;j.moveToElementText(this);j.setEndPoint("StartToEnd",g);h.end=i-j.text.length;j.setEndPoint("StartToStart",g);h.start=i-j.text.length}return h},selectRange:function(g,f){if(Browser.Engine.trident){var h=this.value.substr(g,f-g).replace(/\r/g,"").length;g=this.value.substr(0,g).replace(/\r/g,"").length;var e=this.createTextRange();e.collapse(true);e.moveEnd("character",g+h);e.moveStart("character",g);e.select()}else{this.focus();this.setSelectionRange(g,f)}return this}});Autocompleter.Base=Autocompleter;Autocompleter.Request=new Class({Extends:Autocompleter,options:{postData:{},ajaxOptions:{},postVar:"value"},query:function(){var f=$unlink(this.options.postData)||{};f[this.options.postVar]=this.queryValue;var d=$(this.options.indicator);if(d){d.setStyle("display","")}var e=this.options.indicatorClass;if(e){this.element.addClass(e)}this.fireEvent("onRequest",[this.element,this.request,f,this.queryValue]);this.request.send({data:f})},queryResponse:function(){var c=$(this.options.indicator);if(c){c.setStyle("display","none")}var d=this.options.indicatorClass;if(d){this.element.removeClass(d)}return this.fireEvent("onComplete",[this.element,this.request])}});Autocompleter.Request.JSON=new Class({Extends:Autocompleter.Request,initialize:function(f,d,e){this.parent(f,e);this.request=new Request.JSON($merge({url:d,link:"cancel"},this.options.ajaxOptions)).addEvent("onComplete",this.queryResponse.bind(this))},queryResponse:function(b){this.parent();this.update(b)}});Autocompleter.Request.HTML=new Class({Extends:Autocompleter.Request,initialize:function(f,d,e){this.parent(f,e);this.request=new Request.HTML($merge({url:d,link:"cancel",update:this.choices},this.options.ajaxOptions)).addEvent("onComplete",this.queryResponse.bind(this))},queryResponse:function(d,c){this.parent();if(!c||!c.length){this.hideChoices()}else{this.choices.getChildren(this.options.choicesMatch).each(this.options.injectChoice||function(b){var a=b.innerHTML;b.inputValue=a;this.addChoiceEvents(b.set("html",this.markQueryValue(a)))},this);this.showChoices()}}});Autocompleter.Ajax={Base:Autocompleter.Request,Json:Autocompleter.Request.JSON,Xhtml:Autocompleter.Request.HTML};var Observer=new Class({Implements:[Options,Events],options:{periodical:false,delay:1000},initialize:function(f,e,d){this.element=$(f)||$$(f);this.addEvent("onFired",e);this.setOptions(d);this.bound=this.changed.bind(this);this.resume()},changed:function(){var b=this.element.get("value");if($equals(this.value,b)){return}this.clear();this.value=b;this.timeout=this.onFired.delay(this.options.delay,this)},setValue:function(b){this.value=b;this.element.set("value",b);return this.clear()},onFired:function(){this.fireEvent("onFired",[this.value,this.element])},clear:function(){$clear(this.timeout||null);return this},pause:function(){if(this.timer){$clear(this.timer)}else{this.element.removeEvent("keyup",this.bound)}return this.clear()},resume:function(){this.value=this.element.get("value");if(this.options.periodical){this.timer=this.changed.periodical(this.options.periodical,this)}else{this.element.addEvent("keyup",this.bound)}return this}});var $equals=function(c,d){return(c==d||JSON.encode(c)==JSON.encode(d))};var TocAutoCompleter=new Class({Extends:Autocompleter.Request.JSON,options:{remoteUrl:"json.php",sessionName:"sid",sessionId:null,postData:{module:"auto_completer",action:"get_products"},minLength:3,filterSubset:true,cache:true,delay:0,imageGroup:"thumbnail",moreBtnText:"Get More"},initialize:function(c,d){this.options.postVar=c;this.parent(c,this.options.remoteUrl,d);this.options.postData[this.options.sessionName]=this.options.sessionId;if(d.template){this.options.postData.template=d.template}},showChoices:function(){if(this.options.imageGroup=="thumbnail"){this.choices.addClass("useThumbnail")}var w=this.options.choicesMatch,y=this.choices.getFirst(w);this.selected=this.selectedValue=null;if(this.fix){var u=this.element.getCoordinates(this.relative),x=this.element.getSize(),A;if(this.options.width){this.choices.setStyle("width",this.options.width+"px")}A=this.choices.getSize();if(!this.relative){this.relative=A.x-x.x||x.x-A.x}this.choices.setStyles({left:u.left-this.relative,top:u.bottom});this.choices.addEvent("mouseleave",function(){this.hideChoices()}.bind(this))}if(!y){return}if(!this.visible){this.visible=true;this.choices.setStyle("display","");if(this.fx){this.fx.start(1)}this.fireEvent("onShow",[this.element,this.choices])}if(this.options.selectFirst||this.typeAhead||y.inputValue==this.queryValue){this.choiceOver(y,this.typeAhead)}var v=this.choices.getChildren(w),t=this.options.maxChoices;var p={overflowY:"hidden",height:""};this.overflown=false;if(v.length>t){var q=v[t-1];p.overflowY="scroll";p.height=q.getCoordinates(this.choices).bottom;this.overflown=true}this.choices.setStyles(p);this.fix.show();if(this.options.visibleChoices){var r=document.getScroll(),o=document.getSize(),s=this.choices.getCoordinates();if(s.right>r.x+o.x){r.x=s.right-o.x}if(s.bottom>r.y+o.y){r.y=s.bottom-o.y}window.scrollTo(Math.min(r.x,s.left),Math.min(r.y,s.top))}if(this.choices.getElement("div.more")===null){var z=new Element("div",{"class":"more"}),B=new Element("a",{href:"#","class":"button squre medium btn",html:this.options.moreBtnText});this.choices.adopt(z);z.adopt(B);B.addEvent("click",function(a){a.stop();this.element.getParent("form").submit();return false}.bind(this))}},update:function(c){this.choices.empty();this.cached=c;var d=c&&$type(c);if(!d||(d=="array"&&!c.length)||(d=="hash"&&!c.getLength())){(this.options.emptyChoices||this.hideChoices).call(this)}else{if(this.options.maxChoices<c.length&&!this.options.overflow){c.length=this.options.maxChoices}c.each(this.options.injectChoice||function(h){var i=new Element("li",{html:h,"class":"clearfix"}),b=i.getElement("a"),a=b.get("text"),j=this.markQueryValue(a);i.inputValue=a;b.set("html",j);this.addChoiceEvents(i).inject(this.choices)},this);this.showChoices()}},markQueryValue:function(c){var d=(!this.options.markQuery||!this.queryValue)?c:c.replace(new RegExp("("+((this.options.filterSubset)?"":"^")+this.queryValue.escapeRegExp()+")",(this.options.filterCase)?"":"i"),'<span class="autocompleter-queried">$1</span>');return d},query:function(){this.element.removeClass("auto-loaded").addClass("auto-loading");this.parent()},queryResponse:function(b){this.parent(b);this.element.removeClass("auto-loading").addClass("auto-loaded")},choiceSelect:function(d){var c=d.getElement("a");this.parent(d);window.location=c.getProperty("href")}});var PopupCart=new Class({Implements:[Options],options:{remoteUrl:"json.php",sessionName:"sid",sessionId:null,isCartExpanded:false,triggerEl:$("popupCart"),container:$("pageHeader"),itemsEl:$("popupCartItems"),clsCollapsed:"cartCallpased",clsExpanded:"cartExpanded",clsCartText:"cartText",enableDelete:"yes",enableFlyEffect:true,clsAddBtn:".ajaxAddToCart",clsImage:".productImage",dlgConfirmStatus:true},initialize:function(b){this.setOptions(b);this.registerEvents()},registerEvents:function(){this.options.triggerEl.addEvents({mouseover:function(b){b.stop();if(this.options.isCartExpanded==false){this.getShoppingCart();$$("."+this.options.clsCartText).each(function(a){if(a.hasClass(this.options.clsCollapsed)){a.removeClass(this.options.clsCollapsed)}a.addClass(this.options.clsExpanded)}.bind(this))}}.bind(this)});if(this.options.enableDelete==="no"){this.options.enableFlyEffect=false}if(this.options.enableFlyEffect==true){this.enableFlyingEffects()}},getShoppingCart:function(){var b={action:"get_cart_contents",enable_delete:this.options.enableDelete};this.sendRequest(b,function(a){this.displayCart(a)}.bind(this))},displayCart:function(g){var h=JSON.decode(g),i=this.options.triggerEl.getCoordinates(),l,k,j;if(h.success==true){if(!$defined(this.cartContainer)){this.cartContainer=new Element("div",{html:h.content,id:"popupCartContent",styles:{position:"absolute",opacity:0}});this.cartContainer.addEvent("mouseleave",function(a){a.stop();k.start(1,0);this.options.isCartExpanded=false;$$("."+this.options.clsCartText).each(function(b){if(b.hasClass(this.options.clsExpanded)){b.removeClass(this.options.clsExpanded)}b.addClass(this.options.clsCollapsed)}.bind(this))}.bind(this))}else{this.cartContainer.set("html",h.content)}this.options.container.adopt(this.cartContainer);l=this.cartContainer.getCoordinates();this.cartContainer.setStyles({left:i.left-(l.width-i.width),top:i.top+(i.height/5)});k=new Fx.Tween(this.cartContainer,{duration:100,link:"cancel",property:"opacity"});k.start(0,1);if(this.options.enableDelete=="yes"){j=this.cartContainer.getElements(".removeBtn");if(j.length>0){j.each(function(a){a.addEvent("click",function(b){b.stop();var c=a.get("data-pid");this.removeProduct(c,a);return false}.bind(this))},this)}this.options.itemsEl.set("text",h.total)}this.options.isCartExpanded=true}},enableFlyingEffects:function(){var b=$$(this.options.clsAddBtn);if(b!=null){b.each(function(a){a.addEvent("click",function(p){p.stop();if(a.hasClass("disabled")){return false}a.set("disabled","disabled");var n=[],r=a.get("id"),m=r.test("^ac_[a-z]+_[0-9]+$","i")?r.split("_").getLast():null,t={action:"add_product",pID:m},o=$$("tr.variantCombobox select"),s=$$(".variants_"+m+" select"),e=null,q="";if($defined($("quantity"))){t.pQty=$("quantity").get("value")}if($("qty_"+m)!=null){t.pQty=$("qty_"+m).get("value")}if(o.length>0){e=o}else{if(s.length>0){e=s}}if(e!==null){e.each(function(d){var f=d.id.toString();var c=f.substring(9,f.indexOf("]"));q+=c+":"+d.value+";"}.bind(this));t.variants=q}if($defined($("senders_name"))&&$("senders_name").value!=""){t.senders_name=$("senders_name").value}else{if($defined($("senders_name"))&&$("senders_name").value==""){n.push(this.options.error_sender_name_empty)}}if($defined($("senders_email"))&&$("senders_email").value!=""){t.senders_email=$("senders_email").value}else{if($defined($("senders_email"))&&$("senders_email").value==""){n.push(this.options.error_sender_email_empty)}}if($defined($("recipients_name"))&&$("recipients_name").value!=""){t.recipients_name=$("recipients_name").value}else{if($defined($("recipients_name"))&&$("recipients_name").value==""){n.push(this.options.error_recipient_name_empty)}}if($defined($("recipients_email"))&&$("recipients_email").value!=""){t.recipients_email=$("recipients_email").value}else{if($defined($("recipients_email"))&&$("recipients_email").value==""){n.push(this.options.error_recipient_email_empty)}}if($defined($("message"))&&$("message").value!=""){t.message=$("message").value}else{if($defined($("message"))&&$("message").value==""){n.push(this.options.error_message_empty)}}if($defined($("gift_certificate_amount"))&&$("gift_certificate_amount").value!=""){t.gift_certificate_amount=$("gift_certificate_amount").value}else{if($defined($("gift_certificate_amount"))&&$("gift_certificate_amount").value==""){n.push(this.options.error_message_open_gift_certificate_amount)}}if(n.length>0){alert(n.join("\n"));a.erase("disabled");return false}this.sendRequest(t,function(c){var d=JSON.decode(c);if(d.success==true){this.doFlyingEffects(a,d.items);if(this.options.dlgConfirmStatus==true){this.showConfirmation(d.confirm_dialog)}}else{a.erase("disabled")}});return false}.bind(this))},this)}},doFlyingEffects:function(h,i){var l=$(document.body),j=h.closest(this.options.clsImage),m=j.getCoordinates(),k=j.clone().setStyles({position:"absolute",width:m.width,height:m.height,left:m.left,top:m.top,"z-index":9999}),n=this.options.triggerEl.getCoordinates();l.adopt(k);k.set("morph",{duration:300,onComplete:function(){k.fade("out");(function(){k.destroy()}).delay(500);h.erase("disabled");this.options.itemsEl.set("text",i)}.bind(this)}).morph({width:m.width/4,height:m.height/4,top:n.top,left:n.left})},removeProduct:function(i,l){var h,g=l.getParent("tr"),j=l.getParent("table").getNext("table"),k={action:"remove_product",pID:i};this.sendRequest(k,function(a){h=JSON.decode(a);if(h.success){g.addClass("animated slideOutUp");(function(){g.destroy()}).delay(300);this.options.itemsEl.set("text",h.total);j.set("html",h.order_totals)}}.bind(this))},showConfirmation:function(b){if(b!=null){if(typeof this.dlg!=="undefined"){this.dlg.update(b)}else{this.dlg=new popDialog(b)}this.dlg.show();if($("btnContinue")!=null){$("btnContinue").addEvent("click",function(a){a.stop();this.dlg.doAnimate("slideOutUp");(function(){this.dlg.hide()}.bind(this)).delay(500);return false}.bind(this))}}},sendRequest:function(h,g){var f={template:this.options.template,module:"popup_cart",method:"post"};f[this.options.sessionName]=this.options.sessionId;$extend(f,h);var e=new Request({url:this.options.remoteUrl,data:f,onSuccess:g.bind(this)}).send()}});var TocBookmark=new Class({Implements:[Options],options:{title:document.title,url:window.location.href},initialize:function(f){this.setOptions(f);var g=$(this.options.bookmark);var h=new Element("a");var e=new Element("img",{title:this.options.text,src:this.options.img});if(window.opera&&window.print){h.setProperties({href:this.options.url,title:this.options.title,rel:"sidebar"})}else{h.setStyle("cursor","pointer")}h.appendChild(e);h.appendText(this.options.text);g.appendChild(h);h.addEvent("click",function(){this.onBookmarkClicked()}.bind(this))},onBookmarkClicked:function(){if(window.sidebar){window.sidebar.addPanel(this.options.title,this.options.url,"")}else{if(document.all){window.external.AddFavorite(this.options.url,this.options.title)}}if(window.opera&&window.print){alert("Press ctrl+D to bookmark (Command+D for macs) after you click Ok")}else{if(window.chrome){alert("Press ctrl+D to bookmark (Command+D for macs) after you click Ok")}}}});
