...
Toc.modules_payment.ModulesPaymentGrid = function(config) {
  ...
  config.cm = new Ext.grid.ColumnModel([
    { id: 'modules', header: '<?php echo $osC_Language->get('table_heading_payment_modules'); ?>', dataIndex: 'title'},
  ...
  Toc.modules_payment.ModulesPaymentGrid.superclass.constructor.call(this,config);
};


Ext.extend(Toc.modules_payment.ModulesPaymentGrid, Ext.grid.GridPanel, {
      ...
      case 'icon-uninstall-record':
        if (action == 'icon-uninstall-record') {
          Ext.MessageBox.confirm(
            record.get("title"), 
            '<?php echo $osC_Language->get('introduction_uninstall_payment_module'); ?>',
            function(btn) {
              if (btn == 'yes') {
